...
**Why:** the classifier's only job in the product is choosing a default target class for the warp. The warp maths reads `models/class_means.json`, which shipped at E1, and `core/states.py` rather than the model produces every user-facing string (`PLAN_PROJECT.md` lines 85-87). So the model's accuracy buys exactly one thing: how often the default is already what the user wanted. Across E2 and follow-up probing, everything I can fit on these landmarks sits between kappa 0.155 (the E2 MLP) and kappa 0.248 (gradient boosting on coordinates plus ratios). At that level the default is wrong most of the time, and a wrong default costs the user more than no default: they have to notice it is wrong, then override it. A three-way picker costs one click and is right every time. Shipping the model as a preselection would make the product worse than shipping no model at all.

**Alternative considered:** preselect from the model and show a confidence bar, letting the user correct it. Rejected on the numbers above. That design pays off when the model is right most of the time, and this one is not. Also considered gating the picker behind a confidence threshold, so the model preselects only when confident. Rejected as premature: the model is not calibrated, nothing has measured whether its confident predictions are actually more accurate, and E4 has not yet produced the pose-based confidence penalty that `core/states.py` is supposed to apply.

**How to apply:** build `/edit`'s picker at M3 alongside the warper, not at M5. Treat `models/expression_head.onnx` as an optional enhancement to `/predict`'s response, not a dependency of `/edit`. Research work aimed at raising classifier accuracy should be justified by the research questions Q1-Q5 and the grading rubric, not by product need, because there is no product need. This entry does not say the research track should stop; it says the app does not wait on it, which is what `PLAN_RESEARCH.md`'s decoupling of the two deliverables already intended.

---

## 2026-09-20 — the GNN's readout is a required control in E3, and RSCH-3 goes back for a re-run

**Decided:** RSCH-3 is re-groomed and re-run. Every trained arm uses the same readout over the 48-node axis, and the adjacency is the only thing that varies between arms. The re-groomed issue adds a `gnn_identity` arm (`A_hat = I`, message passing switched off at equal depth and parameter count), matches the balancing treatment across arms, decides the Q1 verdict on kappa rather than macro F1, and carries E2's uniform-dummy reference into E3. The first run's artifacts stay on disk.

**Why:** `_Net.forward` ended in `h.mean(dim=1)`, and the node features come out of `generalized_procrustes`, whose `_center_scale` subtracts each shape's centroid. The node-axis mean of the GNN's input is therefore exactly zero for all 1967 rows in all four channels. Measured between-sample variance surviving that pooling step: 2.58e-33, against 1.64e-2 for the flattened coordinates the E2 MLP reads. The readout threw away the input before the classifier head saw it. That made the GNN differ from the MLP in two ways at once, adjacency and readout, with the readout dominating, so the run could not attribute anything to the graph. Swapping only the readout moves the anatomical arm from 0.266 macro F1 / 0.018 kappa to 0.374 / 0.096, and moves the random-adjacency arm to 0.398 / 0.124, which flips the reported "anatomical beats random" finding. Two further problems compounded it: the GNN was balanced by in-`fit` class weights while the MLP was balanced by oversampling, and the verdict compared two arms both sitting at chance on a metric that cannot distinguish them.

**Alternative considered:** accept Q1 = NO as reported and move on, since the corrected numbers reach the same verdict. Rejected. The verdict survives but its stated reason does not, and the run's "Condition B HOLDS" line claims the anatomical structure carries signal when the corrected comparison says it carries none. `PLAN_RESEARCH.md` asks for exactly that negative by name, so reporting the artifact instead of the finding would throw away the result the experiment existed to produce. Also considered patching the readout and re-running without re-grooming. Rejected because the missing `gnn_identity` control is a grooming defect, not an implementation one, and `research_process.md` puts method-level failures back with the Research PM.

**How to apply:** when a model's preprocessing and its readout are written in different modules, check what the readout does to the preprocessed input before reading any metric. A one-line variance check on the pooled input would have caught this before training. More generally, an architecture comparison changes one thing; if the candidate and the reference differ in two places, the experiment answers neither question. Name the readout as a control at grooming time for any future pooled-representation experiment.

---

## 2026-09-20 — the E3 edge list was revised twice without a record

**Decided:** record, after the fact, that the hand-authored adjacency went through three versions. The entry below this one documents 78 edge lines and 76 unique edges at `models/graph_edges_manual.txt`. That file now lives at `models/graph_edge_schemes/graph_edges_manual.txt`, and two later versions sit beside it: `graph_edges_manual_v2.txt` (108 lines) and `graph_edges_manual_v3.txt` (100 lines, 98 unique, degree 2 to 7, no isolated nodes). `core/graph.py` loads v3. Going forward, a new edge scheme requires an entry here before it becomes the default, stating what changed and why.

**Why:** the edge list is the entire independent variable of E3. Q1 asks whether the anatomical adjacency carries signal, so the adjacency is the thing under test. Revising it twice with no record leaves nobody able to say whether the versions were authored from anatomy or tuned against E3's metrics, and those two histories support opposite readings of the same result. The gap also broke `tests/test_graph.py`, whose edge-count assertion still encoded v1's 76, and left six stale references to the old path across four files that load or describe it: `core/graph.py`, `scripts/gnn.py`, `scripts/graph_review_manual.py` and the E3 README. References to the old path in this file and in `docs/backlog.md` are historical record and stay as they are.

**Alternative considered:** rewrite the entry below to describe v3 and move on, since it is the version in use. Rejected. This file's header says entries are written as decisions are made and the newest goes on top; silently updating an old entry to match current state destroys the record of when the change happened, which is the one thing this file exists to preserve.

**How to apply:** the re-groomed RSCH-3 requires the ML Engineer to state in the run comment whether any scheme was selected by looking at E3 metrics, and to pin the loaded file's path and sha256 in `experiments/e3/config.json`. If a scheme was chosen against the outcome, the comparison is invalid regardless of the numbers and the experiment gets re-scoped again. Keep the superseded versions on disk; they are the audit trail.
