...
## What this is
Six arms on the same 1967 cat-emotions-3 rows and the same frozen five folds E2 and E3 used, each scored inside four bins of a pose estimator built from the landmark map. Method, bin count, verdict metric, thresholds and decision rules are the Research PM's two grooming passes in `docs/backlog.md`, all pre-registered; the constants and both verdict functions were committed in `src/catface/ml/pose.py` and `src/catface/ml/scoring.py` before this run produced a single metric. Produced by `uv run python scripts/pose_and_classes.py`.
...
`d_rel = |s| / tan(theta)` over the 1862 rows above 5 degrees (below that `tan(theta) -> 0` and the ratio is noise): philtrum median **0.0517**, IQR 0.0214 to 0.1026.

Chin control (node 2): median 0.0449, IQR 0.0193 to 0.0913, ratio to the philtrum median 0.87. Synthetic round trip in `tests/test_pose.py`: recovers the built-in 0.30 to 1e-6, chin below 1% of it.

## The manual review montage
`yaw_extremes_manual_review.png`: the most-negative, near-zero and most-positive rows by family 2's signed `s`, cropped to the landmark box and rendered from the source images with points 4, 8, 16 and 2 marked. `yaw_extremes_manual_review.json` names the rows.

Sign balance: 1023 rows negative, 944 positive.

## Legacy centroid proxy
E1's centroid proxy: |proxy| max 0.1265 on these 1967 rows. Correlation against this run's theta: r = -0.067.
...
Control (c), `yaw_only_lr`: logistic regression on the binning quantity alone, same configuration as the other two LR arms, same folds, same oversampling. Pooled kappa 0.041 against the uniform dummy's 0.017.
...
Total-variation distance between each bin's class distribution and the overall prior; the pre-registered tolerance is 0.05.
...
`DummyClassifier(strategy="uniform", random_state=0)` on the identical folds, scored within each bin under that bin's own label prior rather than once over the pooled rows -- see the `random_baseline` per-bin table above.
...
Retrained vs. collapsed vs. uniform-dummy 2-class kappa, **attentive / relaxed** (highest cross-talk pair, candidate for merge: False): retrained 0.148, collapsed 0.190, dummy 0.021 (`merged/gnn_identity/metrics.json`).
